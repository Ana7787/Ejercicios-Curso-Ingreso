...
class App(customtkinter.CTk):
    ...
    def btn_convertir_c_f_on_click(self):
        ...
        centigrados_a_far = float(centigrados)*9/5+32

        alert ( title = "Alert", message =  centigrados+ "°C equivalen a "+str(centigrados_a_far)+ " °F.")



    def btn_convertir_f_c_on_click(self):
        fahrenheit = self.txt_temperatura_f.get()
        fahrenheit_a_cen = float(fahrenheit)-32*5/9
    
        alert ( title = "Alert", message = fahrenheit + " °F equivalen a "+ str(fahrenheit_a_cen)+ " °C")
