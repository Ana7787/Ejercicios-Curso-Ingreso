...
class App(customtkinter.CTk):
    ...
    def btn_calcular_on_click(self):
        ...
        metros_cuadrados = int(largo*ancho)
        metros_lineales = int(largo*2+ancho*2)
        quebracho_grueso = int(metros_lineales/250+4)
        quebracho_fino = int(metros_lineales/12) - quebracho_grueso
        varillas = int(metros_lineales/2)
        alambre = int(metros_lineales*7)

        alert (title = "Alert", message = "El terreno cuenta con " + 
               str(metros_cuadrados) + " metros cuadrados,"+ str(metros_lineales) +" metros lineales, con "+ 
               str(quebracho_grueso) +" postes de quebracho grueso y "+ str(quebracho_fino) + 
               " postes de quebracho fino. En el terreno se encuentran " + str(varillas) + " varillas y "+
               str(alambre) + " metros de alambre alta resistencia 17/15 el cual consta de 7 hilos.")
